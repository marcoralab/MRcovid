...
rg.path <- glue("results/RGcovid/ldsc/all_results_ldsc_{DATE}.tsv")
...
rg_covid <- rg.raw %>%
  ...
  filter(p2 %in% outcomes) %>% 
  nanny::lower_triangular(p1, p2, rg)
```
...
write_csv(., glue("docs/{model}_LDSC_rgRes_{DATE}.csv"))
...
write_csv(MRdat, glue("docs/{model}_HarmonizedDatasets_{DATE}.csv"))
```
...
```{r, eval=TRUE}
write_csv(MRsummary, glue("docs/{model}_MRsummary_{DATE}.csv"))
```
...
write_csv(mr_best, glue("docs/{model}_MRbest_{DATE}.csv"))
```
...
out.final %>% filter(as.numeric(fdr) < 0.05) %>% 
  split(.$outcome)

```

```{r, write-publication-results, echo=FALSE, eval=TRUE}
write_csv(out.final, glue("docs/{model}_PublicationRes_{DATE}.csv"))
```
...
p.leg_out <- ggarrange(ggblank, p.leg, ggblank, 
                      nrow = 1, ncol = 3, widths = c(0.1, 1, 0.025))

## combine heatmap and legend
p.heatmap <- ggarrange(p3, p.leg_out, nrow = 2, ncol = 1, heights = c(1,0.25))
# p.heatmap
```
...
p4 <- ggarrange(p.ls[[1]], ggblank, 
                nrow = 2, ncol = 1, heights = c(1, 1.01))
p6 <- ggarrange(p.ls[[3]], ggblank, nrow = 2, ncol = 1, heights = c(4.9, 1))

p.mr_sig <- ggarrange(ggblank, p4, p.ls[[2]], p6, ggblank, 
                      nrow = 1, ncol = 5, widths = c(0.07, 1, 1, 1, 0.09))
# p.mr_sig <- ggarrange(ggblank, p.ls[[1]], p.ls[[2]], p.ls[[3]], ggblank, 
#                       nrow = 1, ncol = 5, widths = c(0.07, 1, 1, 1, 0.09))
# p.mr_sig
...
# ggsave(glue("docs/{model}_MR_RG_heatmap{DATE}.png"), plot = p3, width = 11, height = 5, units = "in")
ggsave(glue("docs/{model}_resplot_{DATE}.png"), plot = p.comb, height = 190, width = 240, units = 'mm')
ggsave(glue("docs/{model}_resplot_{DATE}.tiff"), plot = p.comb, height = 190, width = 240, units = 'mm')
ggsave(glue("docs/{model}_resplot_{DATE}.pdf"), plot = p.comb, height = 190, width = 240, units = 'mm')
```
...
```{r}
ggsave(glue("docs/{model}_MRheatmap_{DATE}.png"), plot = p2, width = 11, height = 5, units = "in")
ggsave(glue("docs/{model}_RGheatmap_{DATE}.png"), plot = p1, width = 11, height = 5, units = "in")
```
...
```{r}
ggsave(glue("~/Downloads/{model}_covidhgi2020A2v5alleur_forrest_{DATE}.png"), 
       plot = forrest_out.p[[1]], width = 12, height = 6, units = "in")

ggsave(glue("~/Downloads/{model}_covidhgi2020B2v5alleur_forrest_{DATE}.png"), 
       plot = forrest_out.p[[1]], width = 12, height = 6, units = "in")

ggsave(glue("~/Downloads/{model}_covidhgi2020C2v5alleur_forrest_{DATE}.png"), 
       plot = forrest_out.p[[1]], width = 12, height = 6, units = "in")
...
ggsave(glue("docs/{model}_covidhgi2020A2v5alleurLeaveUKBB_forrest_{DATE}.png"), width = 12, height = 6, units = "in")
```
...
ggsave(glue("docs/{model}_covidhgi2020B2v5alleurLeaveUKBB_forrest_{DATE}.png"), width = 12, height = 6, units = "in")
```
...
ggsave(glue("docs/{model}_covidhgi2020C2v5alleurLeaveUKBB_forrest_{DATE}.png"), width = 12, height = 6, units = "in")
```
...
# p.out %>% 
#   ggpubr::ggexport(filename = glue("docs/{model}_MRScatterPlots{DATE}.pdf"),  pointsize = 8, verbose = T)
...
#                            common.legend = TRUE, legend = 'bottom')
# funnel_p.out %>% 
#   ggpubr::ggexport(filename = glue("docs/{model}_MRFunnelPlots{DATE}.pdf"),  pointsize = 8, verbose = T)
...
scatter_funnel.out <- ggpubr::ggarrange(plotlist = scatter_funnel, nrow=3, ncol=2, align = "h",
                          common.legend = TRUE, legend = 'bottom')
scatter_funnel.out %>% 
  ggpubr::ggexport(filename = glue("docs/{model}_MRScatterFunnelPlots_{DATE}.pdf"),  
